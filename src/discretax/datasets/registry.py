...
def _load_uea_preprocessed_splits(
    dataset_root: Path,
    dataset_config: DatasetConfig,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Load UEA split pickles, falling back to legacy merged pickles if needed."""
    split_files = {
        "train_inputs": dataset_root / "X_train.pkl",
        "validation_inputs": dataset_root / "X_val.pkl",
        "test_inputs": dataset_root / "X_test.pkl",
        "train_targets": dataset_root / "y_train.pkl",
        "validation_targets": dataset_root / "y_val.pkl",
        "test_targets": dataset_root / "y_test.pkl",
    }
    if all(path.exists() for path in split_files.values()):
        return (
            _load_pickle(split_files["train_inputs"]).astype(np.float32),
            _labels_to_int(_load_pickle(split_files["train_targets"])),
            _load_pickle(split_files["validation_inputs"]).astype(np.float32),
            _labels_to_int(_load_pickle(split_files["validation_targets"])),
            _load_pickle(split_files["test_inputs"]).astype(np.float32),
            _labels_to_int(_load_pickle(split_files["test_targets"])),
        )

    train_inputs_path = dataset_root / "X_train.pkl"
    ...
    if all(path.exists() for path in (train_inputs_path, train_targets_path, test_inputs_path, test_targets_path)):
        ...
        test_targets = _labels_to_int(_load_pickle(test_targets_path))
        train_split, validation_split = _split_train_validation(
            train_inputs,
            train_targets,
            dataset_config.validation_split,
        ...
        return (
            train_split.inputs.astype(np.float32),
            train_split.targets.astype(np.int32),
            validation_split.inputs.astype(np.float32),
            validation_split.targets.astype(np.int32),
            test_inputs,
            test_targets.astype(np.int32),
        )

    data_path = dataset_root / "data.pkl"
    labels_path = dataset_root / "labels.pkl"
    original_idxs_path = dataset_root / "original_idxs.pkl"
    if data_path.exists() and labels_path.exists():
        data = _load_pickle(data_path).astype(np.float32)
        labels = _labels_to_int(_load_pickle(labels_path))
        if original_idxs_path.exists():
            original_train_idx, original_test_idx = _load_pickle(original_idxs_path)
            train_inputs = data[np.asarray(original_train_idx)]
            train_targets = labels[np.asarray(original_train_idx)]
            test_inputs = data[np.asarray(original_test_idx)]
            test_targets = labels[np.asarray(original_test_idx)]
            train_split, validation_split = _split_train_validation(
                train_inputs,
                train_targets,
                dataset_config.validation_split,
        ...
        return (
            ...
            train_targets.astype(np.int32),
            validation_inputs.astype(np.float32),
            validation_targets.astype(np.int32),
            test_inputs.astype(np.float32),
            test_targets.astype(np.int32),
        )

    raise FileNotFoundError(
        "Could not find preprocessed UEA split pickles in "
        f"{dataset_root}. Expected X_{{train,val,test}}.pkl and y_{{train,val,test}}.pkl."
    )
...
def _build_uea_dataset(paths_config: PathsConfig, dataset_config: DatasetConfig) -> DatasetBundle:
    ...
    (
        ...
        test_inputs,
        test_targets,
    ) = _load_uea_preprocessed_splits(dataset_root, dataset_config)
