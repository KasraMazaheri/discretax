"""Launch the default ETTh1 non-patch LinOSS head sweep for one family on one node."""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help="Which ETTh1 LinOSS family to launch on this node.",
    )
    parser.add_argument(
        "--sweep-config",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Concurrent jobs to allow per GPU.",
    )
    parser.add_argument(
        "--log-dir",
        default="logs",
...
def _tmux_session_name(job_name: str, index: int) -> str:
    ...
    return f"dx-etth1-{index:02d}-{slug}"
...
def main() -> None:
    """Launch one default ETTh1 non-patch head sweep family."""
    args = _parse_args()
